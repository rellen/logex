...
vocabulary. 555 lines of hand-written source — `lib/` + `src/*.xrl` +
...
- **Case — mnemonics case-insensitive, tags case-sensitive.** One `String.downcase/1` at
  `compiler.ex:32`. IEC and every vendor are case-*insensitive*, so a user arriving from
...
- **`mov` → `move`.** The one existing name the survey changed. The mainstream North
  American toolchain renamed MOV→MOVE in its 2024 conformance sweep *"to conform to IEC
