...
Status of this document: written 2026-08-30 against commit `c9c7f51`, then revised after
an adversarial review of the plan itself. It records the findings of a full review of the
codebase and turns them into an ordered plan.
...
instruction vocabulary. 407 lines of hand-written source — `lib/` + `src/*.xrl` +
`src/*.yrl` + `test/` + `mix.exs`, excluding the generated `src/*.erl` — and no
dependencies. The front end is generated by leex and yecc; everything downstream lives in
one 121-line module.
...
**The trailing-newline case cannot pass until M0-5, three items later.** Tag it
`@tag :skip` and drop the tag in the M0-5 commit, so M0-3 and M0-4 are done against a
green suite — `CLAUDE.md:8` says tests must pass before committing, and M0-3's whole
subject is a build trap that a trustworthy `mix test` is what catches.
...
Then correct `CLAUDE.md:7` to read:
...
And extend `CLAUDE.md:20` — its new-instruction recipe omits that the de-energised
`{false, env}` clause is **mandatory**. Following it literally yields code that works on
an energised rung and raises `FunctionClauseError` the moment a contact opens. (§4's
guard-clause collapse does not retire this: see the Style bullet.)
...
**Do M0-3 first**, and run `rm -f src/*.erl && rm -rf _build` before every compile in
M0-4 and M0-5 whether or not M0-3 has landed. M0-4 and M0-5 are the only items that edit
`.xrl`/`.yrl`, and M0-3 removes the fresh-clone case, not the mtime gate — a grammar edit
in the same wall-clock second as the last build is silently ignored, with no output and a
green suite.
...
newline-naive grammar; pick one.
...
**After M0-1 … M0-5 a green `mix test` means something:** every stage boundary is
exercised from source, and the three things §1 lists as broken — integer literals, sources
read from a file, single-character tags — actually work. This is the *testability* gate,
not the correctness gate. Three med-severity defects that produce silently wrong answers
...
reaches ten pattern sites in `lib/logex/compiler.ex` — `[{:name, name} | tail]` (`:31`),
the six `[name: arg]` clause heads (`:71,:79,:87,:91,:95,:103`), `{:name, arg2}` (`:111`)
and both `get_arg/2` clauses (`:119-120`) — plus the fixtures in all three test files. The
...
else — while `elem -> int_lit` (`ladder_parser.yrl:19`) makes a literal in opcode position
...
```elixir
# Replaces the two TRUE-rung clauses only — compiler.ex:71-73 and :79-81. Keep the
# {false, env} clauses at :75-77 and :83-85; deleting them makes any de-energised
# xic/xio raise FunctionClauseError while `mix test` stays green.
...
(`compiler.ex:120`) is a bare `Map.get/2`, so `mov undefined_tag dst` stores `nil` today.
...
bug fix: real Logix takes a BOOL operand and rejects `XIC(setpoint)` on a DINT at
verification. Adopt it as an interim rule and record the choice in §5.

**Do not land M1-4 before M1-3** — the `0` default is only safe once an undeclared tag is
a compile error rather than a silent zero.
...
from an empty token list, and M0-5 makes the empty program a legal parse. So by the time
...
**Settle §5 first.** Timer addressing does not lex today — `tokenize("xic Timer_1.DN")`
returns `{:illegal, '.'}` — so the surface-syntax decision is a prerequisite, not a
follow-up. Take the elapsed time as an *injected scan input* rather than an internal
...
- **B1 · Punctuation branch delimiters.** A missing space before `nxb` silently turns OR into
  ...
  **Gated on §5, and a source-language break — not a contained lexer tweak.**
  `bst`/`nxb`/`bnd` stop being keywords and become ordinary tag names, so every branching
  program has to be rewritten: `lex_and_parse_test.exs:6`, whichever M0-2 seeds branch,
  and the branching examples in this document. The migration is *silent*, not loud — an
  ...
  the exact failure M1-2 exists to remove. And this borrows neutral text's brackets
  without its parentheses: once M0-4 frees `[`/`]` from `NAME`, they belong to branch
  structure alone, so the `[3]` addressing wanted in §5 and §8·6 becomes unlexable
  (verified: `tokenize("xic aa[3]")` → `name, name, bst, int_lit, bnd`). Real neutral text
  avoids the clash only because operands are parenthesised — `XIC(aa[3])`. Settle §5
  first, or pick delimiters that leave `[`/`]` free; the fix only needs a delimiter that
  cannot fuse into an identifier.
...
- **B3 · CI.** Four lines: `mix compile --warnings-as-errors`,
  ...
  All four pass today. Note the image needs leex and yecc once M0-3 lands (see its
  tradeoff note), and M0-2 seeds a test that is red until M0-5.
  ...
  have a clean baseline. Of the 8 commits only 6 carry a mix project at all.
...
- **B7 · Style.** Five `{false, env}` clauses with identical *bodies*
  (`compiler.ex:75,83,99,107,115` — the heads differ) collapse to one guard clause
  ...
  the `CLAUDE.md:20` trap, it *moves* it — verified: add a new opcode per that recipe and
  a de-energised rung still raises `FunctionClauseError`, because the new atom was never
  added to the whitelist. So M0-3's doc fix still stands, reworded to "add the opcode to
  the `{false, env}` guard list, or give it its own `{false, env}` clause if
  de-energising does something, as `ote` does". Do not widen the guard to a bare
...
§4·B1's punctuation branch delimiters are the first concrete fork in this decision:
taking them alone is a half-step into neutral text that closes off `[3]` addressing.
M1-4's nonzero-is-true rule for `xic`/`xio` is the second — real Logix rejects a non-BOOL
operand at verification rather than coercing it.
...
it. `B1`–`B7` are the §4 backlog bullets. Items that are pure forward work rather than
defects (M1-1, M1-3, M1-6) have no row.
...
| B1 | med | lexer | Missing space before `nxb` fuses into an identifier — parallel silently becomes series | `ladder_lexer.xrl:8` |
...
| M1-5 | low | errors | Three error conventions across four stages; `format_error/1` never called; empty program reports line `999999` — closed by M0-5 | `compiler.ex:2-8` |
...
| B4 | low | tooling | nix dev shell bit-rotted: `erlangR26` alias removed from nixpkgs master 2024-05-24 (still in `nixos-24.05`, gone in `24.11`); flake tracks `master`; lock is 31 months old | `shell.nix:4-5` |
| B2 | low | lexer | Digit-led lexeme splits rather than erroring: `1bst` → `int_lit(1)` + a real `bst` | `ladder_lexer.xrl:18` |
| B3 | low | history | 2 of the 6 commits with a mix project ship a red suite (`ec0534a`, `35fe1b9`) — an interior island; `d47eb21` is a clean bisect baseline | — |
| B6 | low | project | No CI, one-line README, no `@spec`/`@moduledoc`, no mix.exs metadata, unused `:logger` | `mix.exs` |
| B7 | nit | style | 5 `{false, env}` clauses with identical bodies; `&f(&1)`; `Enum.any?(o, &(&1==true))`; intermediate list in branch reducer | `compiler.ex:56-61,75-115` |
| M1-1 | nit | IR | AST nodes are keyword-list-shaped with duplicate keys where order is the meaning; `Keyword.get/2` would silently return only the first. Closed by M1-1's 3-tuple elem | `ladder_parser.yrl:11-23` |
