...
vocabulary. 681 lines of hand-written source — `lib/` + `src/*.xrl` +
...
`26 tests, 0 failures`. M1-1 has landed; the next unstarted work is §3·M1-2.
...
in the `[{:name, _line, name} | tail]` clause of `instructionize/1` destructures `nil` for any
...
- **B3 · CI.** Four lines: `mix compile --warnings-as-errors`,
  ...
  All four pass today, still true after M1-1 — now `26 tests, 0 failures`, and the
