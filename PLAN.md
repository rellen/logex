...
adversarial review of the plan itself, and annotated against commit `863af6f` once
Milestone 0 was implemented and merged (PRs #3 and #4). It records the findings of a full
...
instruction vocabulary. 555 lines of hand-written source — `lib/` + `src/*.xrl` +
...
yecc; everything downstream lives in one 121-line module.
...
`1 doctest, 19 tests, 0 failures`. The next unstarted work is §3.
...
contemporaneous run; the suite is now 19 tests.
...
and both `get_arg/2` clauses (`:119-120`) — plus the elem fixtures in the three per-stage
...
- **B1 · Punctuation branch delimiters.** A missing space before `nxb` silently turns OR into
  ...
  loud error. M0-4 was the prerequisite and has landed; before it, `a-zA-z` in `NAME`
  swallowed `]` into identifiers.
...
- **B3 · CI.** Four lines: `mix compile --warnings-as-errors`,
  ...
  All four pass today, still true at `863af6f` — `1 doctest, 19 tests, 0 failures`, and the
...
|---|---|---|---|---|---|
| M0-1 | high | evaluate | `get_arg/2` matches `:lit_int`; every other stage emits `:int_lit` | `compiler.ex:119` | **closed** `690fc2d` |
...
| M1-2 | med | lowering | No validation pass: unknown mnemonic → bare `MatchError`; short arity → truncated IR | `compiler.ex:32,35` | open |
...
| B6 | low | project | No CI, one-line README, no `@spec`/`@moduledoc`, no mix.exs metadata, unused `:logger` | `mix.exs` | open |
| B7 | nit | style | 5 `{false, env}` clauses with identical bodies; `&f(&1)`; `Enum.any?(o, &(&1==true))`; intermediate list in branch reducer | `compiler.ex:56-61,75-115` | open |
