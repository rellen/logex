...
vocabulary. 646 lines of hand-written source — `lib/` + `src/*.xrl` +
...
`1 doctest, 24 tests, 0 failures`. The next unstarted work is §3.
...
contemporaneous run; the suite is now 24 tests.
...
- **B3 · CI.** Four lines: `mix compile --warnings-as-errors`,
  ...
  All four pass today, still true at `863af6f` — now `1 doctest, 24 tests, 0 failures`, and the
