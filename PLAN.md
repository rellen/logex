...
vocabulary. 644 lines of hand-written source — `lib/` + `src/*.xrl` +
...
`25 tests, 0 failures`. The next unstarted work is §3.
...
- **B3 · CI.** Four lines: `mix compile --warnings-as-errors`,
  ...
  All four pass today, still true after the `Logex` stub was deleted — now `25 tests, 0 failures`, and the
